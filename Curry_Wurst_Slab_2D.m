...
if strcmp(Slab.Type,'Ribe_Mode')
    ...
    % Compute the top surface of the slab 
    z_top_f =-tan(theta*pi/180).*L0 ;
    % Compute  the lowest point of the surface
    P_B  = [L0-sin(theta*pi/180), z_top_f-cos(theta*pi/180)*D0];
    % Select the point that are within a reasonable sub space
    % Compute the slope of the thickness  (i.e. from the last point of the 
    line_m = (z_top_f-P_B(2))./(C(1)+L0-P_B(1)); %
    % Find the index of the choosen one 
    ip = z_p >= C(2)-Slab.L0 & z_p<=0.0 & x_p>=C(1) & x_p <= C(1)+L0 & z_p>=line_m.*(x_p-P_B(1))+P_B(2);
    % select the chosen one 
    x_pb = x_p(ip==1); 
    z_pb = z_p(ip==1); 
    [z_top,dz,~,~] = compute_ribe_angles(x_pb,C,theta,Slab.L0,D0,weak_flag,Slab.tk_WZ,C(1)+L0);
    % Compute the position x of the projection of the points
    x_t(ip==1) = (z_top-z_pb)./sqrt(1+dz.^2);
    Layout     = -x_t; 
    if weak_flag == 0
        Layout(Layout>0|Layout<-D0)=nan; 
    else
        Layout(Layout>0|Layout<-Slab.tk_WZ)=nan; 
    end
...
%             lx = ind_x(ix);
%             lz = ind_z(iz);
%             %find relevant surface
%             [z_mid,dz,~,~] = compute_ribe_angles(A.Xpart(:,lx,lz),C,theta,Slab.L0,D0,weak_flag,Slab.tk_WZ,C(1)+L0);
%       
%             if  A.Zpart(:,lx,lz) >= line_m.*(A.Xpart(:,lx,lz)-P_B(1))+P_B(2)
%             
%                 x_t = (z_mid-A.Zpart(:,lx,lz))./sqrt(1+dz.^2);
%                 Layout(:,lx,lz)        =-x_t;
%                 if z_top>=D_c & A.Zpart(:,lx,lz)>=D_c
%                     continent(:,lx,lz)          = 1.0; 
%                 end
%                
...
function [z,dz,ztop,zbot] = compute_ribe_angles(xp,C,teta_0,L,D0,weak_flag,Tk,Lim)
%=========================================================================%
